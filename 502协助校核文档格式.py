...
def images_to_word(txt_path, output_word, images_folder=".", chapter_num=1):
    ...
    for group_num, line in enumerate(lines, 1):
        ...
        main_table = doc.add_table(rows=9, cols=3)
        ...
        # 第7行：失配结果标题
        cell_mismatch_title = main_table.cell(6, 0)
        cell_mismatch_title.merge(main_table.cell(6, 2))
        set_cell_text(cell_mismatch_title, "失配图结果", font_size=Pt(12), bold=True)

        # 第8行：列标题（只有2列）
        set_cell_text(main_table.cell(7, 0), "河南大学", bold=True)
        set_cell_text(main_table.cell(7, 1), "香港理工大学", bold=True)
        # 第3列留空或合并到第2列
        main_table.cell(7, 2).text = ""

        # 第9行：2张失配图（索引6, 7）
        for i in range(2):
            img_path = os.path.join(images_folder, image_names[6 + i])
            add_image_to_cell(main_table.cell(8, i), img_path)
        # 第3列留空
        main_table.cell(8, 2).text = ""
