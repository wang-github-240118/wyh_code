...
def rename_images(csv_file_path, images_dir):
    """
    根据CSV文件中的信息重命名影像文件，并移动到自动创建的新文件夹
    新文件夹会在images_dir下自动创建，命名为「重命名文件」

    参数:
    csv_file_path: CSV文件路径
    images_dir: 影像文件所在文件夹路径
    """
    # 🔥 核心改进1：定义新文件夹路径（在影像文件夹下创建）
    new_folder = os.path.join(images_dir, "重命名文件")
    # 自动创建新文件夹（exist_ok=True：存在则不报错，不存在则创建）
    try:
    ...
    for image_path in image_files:
        ...
        if matched_nac:
            ...
            # 🔥 核心改进2：新文件路径指向「重命名文件」文件夹，而非原文件夹
            new_filepath = os.path.join(new_folder, new_filename)

            # 执行重命名+移动（os.rename可实现跨文件夹移动，原文件会保留）
            try:
                os.rename(image_path, new_filepath)
                print(f"已重命名并移动: {image_filename} -> {new_folder}/{new_filename}")
                renamed_count += 1
            except Exception as e:
                print(f"重命名移动 {image_filename} 时出错：{str(e)}")
                skipped_count += 1
        else:
    ...
    print(f"\n✅ 重命名移动完成！")
    print(f"📊 统计结果：成功 {renamed_count} 个 | 跳过 {skipped_count} 个")
    print(f"📁 所有重命名文件已保存至：{new_folder}")
